...
export default function BudgetPage() {
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      // 1. Stock Management
      const products = await safeQuery('products', 'id, name, quantity, unit_price');
      data.stock = {
        totalProducts: products.length || 0,
        totalQuantity: products.reduce((s: number, p: any) => s + (p.quantity || 0), 0) || 0,
        totalValue: products.reduce((s: number, p: any) => s + ((p.quantity || 0) * (p.unit_price || 0)), 0) || 0,
        lowStock: products.filter((p: any) => (p.quantity || 0) < 10).length || 0,
        details: products,
      };
