...
export default function ReceiveProductsPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [receptionsData, suppliersData, categoriesData, unitiesData] = await Promise.all([
  ...
  const handlePrintReception = (reception: ReceptionProduct) => {
    ...
    const html = `
      ...
      <head>
        <meta charset="utf-8">
        <title>Reception ${reception.reception_id}</title>
        <style>
      ...
      <body>
        <div class="header">
          <div class="company-info">
            <h1>${enterpriseSettings?.name || 'Enterprise'}</h1>
            <p>${enterpriseSettings?.description || ''}</p>
          </div>
          ${enterpriseSettings?.logoUrl ? `<img src="${enterpriseSettings.logoUrl}" class="logo" alt="Logo">` : ''}
        </div>

        <h2 style="color: #1e40af; margin-bottom: 20px;">RÉCEPTION PRODUITS - ${reception.reception_id}</h2>

        <div class="reception-details">
          <div class="detail-item">
            <h3>Fournisseur</h3>
            <p>${reception.supplier_name}</p>
          </div>
          <div class="detail-item">
            <h3>Date</h3>
            <p>${new Date(reception.reception_date).toLocaleDateString()}</p>
          </div>
          <div class="detail-item">
            <h3>Statut</h3>
            <p>${reception.status.toUpperCase()}</p>
          </div>
        </div>
          ...
          <tbody>
            ${receptionItems.map(item => `
              <tr>
                ...
                <td>${item.quantity}</td>
                <td>${item.price_per_unity.toLocaleString()} DA</td>
                <td>${(item.quantity * item.price_per_unity).toLocaleString()} DA</td>
              </tr>
            `).join('')}
            <tr class="total-row">
              <td colspan="2"></td>
              <td>TOTAL:</td>
              <td>${reception.total_price.toLocaleString()} DA</td>
            </tr>
          </tbody>
        </table>

        ${reception.notes ? `<div style="margin: 20px 0; padding: 15px; background: #f3f4f6; border-radius: 8px;">
            ...
            >
              ...
              <div className="flex gap-2 flex-wrap">
                <Button
                  ...
                  onClick={() => handlePrintReception(reception)}
                >
        ...
        <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
          <DialogHeader className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-slate-800 dark:to-slate-900 -mx-6 -mt-6 px-6 py-6 mb-6 rounded-t-lg border-b border-blue-200 dark:border-slate-700">
            <div className="flex items-center justify-between">
              ...
              {viewReception && (
                <Button
                  onClick={() => handlePrintReception(viewReception)}
                  className="gap-2 btn-gradient font-semibold"
                >
